...
library(packfor) # Available on R-Forge, 
	...
	# of packfor, so users must install the gfortran compiler. Choose ÔMacOS XÕ 
	# in the Ôcran.r-project.orgÕ window, then ÔtoolsÕ.
library(MASS)
library(ellipse)
library(FactoMineR)
...
# 3. Forward selection using packforÕs forward.sel()
# library(packfor) # if not already loaded
forward.sel(spe.hel, env, adjR2thresh=R2a.all)

# Forward selection using veganÕs ordistep(). 
...
windows(title="Variation partitioning - parsimonious subset 2")
...
windows(title="RDA w. 2nd-order variables - scaling 2")
...
windows(title="Four fish species", 9, 9, )
par(mfrow=c(2, 2))
...
windows(title="Discriminant analysis")
...
chem.topo.ccora <- CCorA(envchem2, envtopo2, stand.Y=TRUE, stand.X=TRUE, 
  nperm=999)
chem.topo.ccora
windows(title="Canonical correlation analysis", 12,9)
...
coia.chem.topo
coia.chem.topo$eig[1]/sum(coia.chem.topo$eig) # Relative variation on first eigenvalue
summary(coia.chem.topo)
randtest(coia.chem.topo, nrepet=999)    # Permutation test

windows(title="Co-inertia analysis")
plot(coia.chem.topo)
...
windows(title="MFA: correlations among significant variables")
